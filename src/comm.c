...
struct boot_cmd_args_data* parseCommandLine(int argc, char **argv) {
	int argPos = 1;
	struct boot_cmd_args_data* cmdlineArgsData = (struct boot_cmd_args_data*) malloc(sizeof(struct boot_cmd_args_data));

	/* Init struct data*/
	cmdlineArgsData->runUnityTest = 0;
	cmdlineArgsData->exitAfterTests = 0;
	cmdlineArgsData->port = 0;
	cmdlineArgsData->mother_desc = 0;
	cmdlineArgsData->fCopyOver = 0;
	cmdlineArgsData->miniMudMode = 0;
	cmdlineArgsData->syntaxCheckMode = 0;
	cmdlineArgsData->noRentCheckMode = 0;
	cmdlineArgsData->restrictMode = 0;
	cmdlineArgsData->noSpecialsMode = 0;
	cmdlineArgsData->displayHelp = 0;
	cmdlineArgsData->errorFound = 0;
	cmdlineArgsData->configFileName = NULL;
	cmdlineArgsData->configFileName = NULL;
	...
	while ((argPos < argc) && (*(argv[argPos]) == '-') && (cmdlineArgsData->errorFound == 0) ) {
		...
		case 'f':
			if (*(argv[argPos] + 2)) {
				...
				cmdlineArgsData->configFileName = argv[argPos];
			}
			else {
				cmdlineArgsData->errorFound = 1;
				cmdlineArgsData->errorDescription = strdup("SYSERR: File name expected after option -f.");
			}
			break;
		case 't':
			cmdlineArgsData->runUnityTest = TRUE;
			break;
		case 'T':
			cmdlineArgsData->runUnityTest = TRUE;
			cmdlineArgsData->exitAfterTests = TRUE;
			break;
		case 'o':
			if (*(argv[argPos] + 2)) {
				cmdlineArgsData->logFileName = argv[argPos] + 2;
			}
			else if (++argPos < argc) {
				cmdlineArgsData->logFileName = argv[argPos];
			}
			else 
			{
				cmdlineArgsData->errorFound = 1;
				cmdlineArgsData->errorDescription = strdup("SYSERR: File name to log to expected after option -o.");
			}
			break;
		case 'C': /* -C<socket number> - recover from copyover, this is the control socket */
			cmdlineArgsData->fCopyOver = TRUE;
			cmdlineArgsData->mother_desc = (socket_t)atoi(argv[argPos] + 2);
			break;
		case 'd':
			if (*(argv[argPos] + 2)) {
				cmdlineArgsData->dataFolder = argv[argPos] + 2;
			}
			else if (++argPos < argc) {
				cmdlineArgsData->dataFolder = argv[argPos];
			}
			else {
				cmdlineArgsData->errorFound = 1;
				cmdlineArgsData->errorDescription = strdup("SYSERR: Directory arg expected after option -d.");
			}
			break;
		case 'm':
			cmdlineArgsData->miniMudMode = 1;
			break;
		case 'c':
			cmdlineArgsData->syntaxCheckMode = 1;
			break;
		case 'q':
			cmdlineArgsData->noRentCheckMode = 1;
			break;
		case 'r':
			cmdlineArgsData->restrictMode = 1;
			break;
		case 's':
			cmdlineArgsData->noSpecialsMode = 1;
			break;
		case 'h':
			cmdlineArgsData->displayHelp = 1;
			break;
		default:
			cmdlineArgsData->errorFound = 1;
			sprintf(cmdlineArgsData->errorDescription, "SYSERR: Unknown option -%c in argument string.\n", *(argv[argPos] + 1));
			//cmdlineArgsData->errorDescription = strdup("SYSERR: Unknown option -%c in argument string.\n", *(argv[argPos] + 1));
			break;
		}
		argPos++;
	}
	...
	if (argPos < argc) {
		if (!isdigit(*argv[argPos])) {
			cmdlineArgsData->errorFound = 1;
			sprintf(cmdlineArgsData->errorDescription, "Usage: %s [-c] [-m] [-q] [-r] [-s] [-d pathname] [port #]\n", argv[0]);
		}
		else if ((cmdlineArgsData->port = atoi(argv[argPos])) <= 1024) {
			cmdlineArgsData->port = 0;
			cmdlineArgsData->errorFound = 1;
			sprintf(cmdlineArgsData->errorDescription, "SYSERR: Illegal port number %d. Port number must be greater than 1024 \n", port);			
		}
	}
...
int main(int argc, char **argv)
{
  ...
  * use the default config file lib/config
  */
  ...
  if (CONFIG_CONFFILE == NULL)  {
	  puts(" Error loading configuration. Aborting. \r\n");
	  exit(1);
  }
  printf("Configuration File %s \r\n" , config_info.CONFFILE );

  /* 
  Load configuration based on config file given

  - We load the default configuration first, so it will be 
  overwritten by the paramters/custom config.
  ...
  */
  ...
  /*
  * Setup the custom config parameters (command line)
  */
  ...
  {
	  dir = cmdData->dataFolder;
  }
  if (cmdData->fCopyOver == TRUE) {
	  fCopyOver = cmdData->fCopyOver;
	  mother_desc = cmdData->mother_desc;  
  }
  if (cmdData->miniMudMode == TRUE) {
	  mini_mud = cmdData->miniMudMode;
	  puts("Running in minimized mode & with no rent check.");
  }
  if (cmdData->noRentCheckMode == TRUE) {
	  no_rent_check = cmdData->noRentCheckMode;
	  puts("Quick boot mode -- rent check supressed.");
  }
  if (cmdData->restrictMode == TRUE) {
	  circle_restrict = cmdData->restrictMode;
	  puts("Restricting game -- no new players allowed.");
  }
  if (cmdData->syntaxCheckMode == TRUE) {
	  scheck = cmdData->syntaxCheckMode;
	  puts("Syntax check mode enabled.");
  }
  if (cmdData->noSpecialsMode == TRUE) {
	  no_specials = cmdData->noSpecialsMode;
...
static void signal_setup(void)
{
  ...
  my_signal(SIGPIPE, SIG_IGN); // IGNORE
  my_signal(SIGALRM, SIG_IGN); // IGNORE
}
